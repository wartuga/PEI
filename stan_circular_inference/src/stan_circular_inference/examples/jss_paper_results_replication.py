...
percentage (e.g. 0%).
Could not upload the dataset file due to its size but the URL is:
https://data.sintef.no/feature/fe-000d522b-9de6-47c0-a615-03cfd3a0fe53
...
To run the script:
 ...
 - Add this script to path "src/stan_circular_inference/examples"
...
mu_real = np.pi
kappa_real = 5
data = vonmises.rvs(kappa_real, loc=mu_real, size=20)
...
# could not upload the dataset file due to its size but the URL is: https://data.sintef.no/feature/fe-000d522b-9de6-47c0-a615-03cfd3a0fe53
# file: eddypro_Ane1-50m-run1_full_output_2025-06-29T132103_exp.csv
# rename to "wind_dataset.csv" and put it in the "datasets" folder
...
values = service.get_values(fit, parameters=['mu1'])['mu1']
    ...
    for kappa in kappa_values:
        for dataset_size in dataset_sizes:
            ...
            for _ in range(n_iter):
                samples_list.append((
                    vonmises.rvs(loc=mu1, kappa=kappa, size=size1),
                    vonmises.rvs(loc=mu2, kappa=kappa, size=size2)
                ))
            ...
            for samples_tuple in samples_list:
                ...
                if mixture_stats['accuracy'] < 1.0:
                    ...
                    # label_switching.png
                    service.match_points_to_distributions(samples, real_dist, inferred_labels, min_val=0, max_val=2*np.pi, data_type=DataType.RADS)
...
samples = vonmises.rvs(real_kappa, loc=real_mu, size=size)
...
# stats_table.png
service.get_pystan_statistics(model_data, parameters=['mu', 'kappa'], sample_amount=10000)